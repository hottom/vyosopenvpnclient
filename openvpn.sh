...
generateinterface() {
    ...
    CLIENTPREFIXSTART="${ip_parts[0]}.${ip_parts[1]}"
    P2PIPPREFIXSTART="10.8.${interefaceID}"
}
...
srctopublic() {
	...
	echo "iroute $CLIENTPREFIXSTART.0.0 255.255.0.0" >> /etc/openvpn/ccd/$client
	detect_ip
	echo "server ip is "$ip
	INTERFACE=`ip address | grep "$ip" | awk '{print $NF}'`
	echo "server INTERFACE is "$INTERFACE
	ip_list=$(ip -4 addr | grep inet | grep "$INTERFACE" | cut -d '/' -f 1 | grep -oE '[0-9]{1,3}(\.[0-9]{1,3}){3}')
	ExecStartpolicy="ExecStart=$iptables_path -w 5 -I FORWARD -s $CLIENTPREFIXSTART.0.0/16 -j ACCEPT"
	ExecStoppolicy="ExecStop=$iptables_path  -w 5 -D FORWARD -s $CLIENTPREFIXSTART.0.0/16  -j ACCEPT"
	CLIENTIPQTY=0
	currip="$CLIENTIPSTART"
	while IFS= read -r line; do
		startpolicy="ExecStart=$iptables_path -w 5 -t nat -A POSTROUTING -s $currip -o $INTERFACE  --dst 0.0.0.0/0  -j SNAT --to-source $line"
		stoppolicy="ExecStop=$iptables_path  -w 5 -t nat -D POSTROUTING -s $currip -o $INTERFACE  --dst 0.0.0.0/0  -j SNAT --to-source $line"
		ExecStartpolicy="$ExecStartpolicy\n$startpolicy"
		ExecStoppolicy="$ExecStoppolicy\n$stoppolicy"
		currip=$(increment_ip "$currip" "1")
		((CLIENTIPQTY++))
	done <<< "$ip_list"	
	export CLIENTIPQTY
	ExecStartpolicy="$ExecStartpolicy\n"
	ExecStoppolicy="$ExecStoppolicy\n"
	echo -e $ExecStartpolicy
	echo -e $ExecStoppolicy
}
...
create_firewall_rules() {
	if systemctl is-active --quiet firewalld.service; then
		...
		firewall-cmd -q --zone=trusted --add-source=${P2PIPPREFIXSTART}.0/24
		firewall-cmd -q --permanent --add-port="$port"/"$protocol"
		firewall-cmd -q --permanent --zone=trusted --add-source=${P2PIPPREFIXSTART}.0/24
		# Set NAT for the VPN subnet
		firewall-cmd -q --direct --add-rule ipv4 nat POSTROUTING 0 -s ${P2PIPPREFIXSTART}.0/24 ! -d ${P2PIPPREFIXSTART}.0/24 -j MASQUERADE
		firewall-cmd -q --permanent --direct --add-rule ipv4 nat POSTROUTING 0 -s ${P2PIPPREFIXSTART}.0/24 ! -d ${P2PIPPREFIXSTART}.0/24 -j MASQUERADE
		if [[ -n "$ip6" ]]; then
...
Type=oneshot
$(echo -e $ExecStartpolicy)
ExecStart=$iptables_path -w 5 -t nat -A POSTROUTING -s ${P2PIPPREFIXSTART}.0/24 ! -d ${P2PIPPREFIXSTART}.0/24 -j MASQUERADE
ExecStart=$iptables_path -w 5 -I INPUT -p $protocol --dport $port -j ACCEPT
ExecStart=$iptables_path -w 5 -I FORWARD -s ${P2PIPPREFIXSTART}.0/24 -j ACCEPT
ExecStart=$iptables_path -w 5 -I FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT
$(echo -e $ExecStoppolicy)
ExecStop=$iptables_path -w 5 -t nat -D POSTROUTING -s ${P2PIPPREFIXSTART}.0/24 ! -d ${P2PIPPREFIXSTART}.0/24 -j MASQUERADE
ExecStop=$iptables_path -w 5 -D INPUT -p $protocol --dport $port -j ACCEPT
ExecStop=$iptables_path -w 5 -D FORWARD -s ${P2PIPPREFIXSTART}.0/24 -j ACCEPT
...
remove_firewall_rules() {
	...
	if systemctl is-active --quiet firewalld.service; then
		ip=$(firewall-cmd --direct --get-rules ipv4 nat POSTROUTING | grep '\-s ${P2PIPPREFIXSTART}.0/24 '"'"'!'"'"' -d ${P2PIPPREFIXSTART}.0/24' | grep -oE '[^ ]+$')
		# Using both permanent and not permanent rules to avoid a firewalld reload.
		firewall-cmd -q --remove-port="$port"/"$protocol"
		firewall-cmd -q --zone=trusted --remove-source=${P2PIPPREFIXSTART}.0/24
		firewall-cmd -q --permanent --remove-port="$port"/"$protocol"
		firewall-cmd -q --permanent --zone=trusted --remove-source=${P2PIPPREFIXSTART}.0/24
		firewall-cmd -q --direct --remove-rule ipv4 nat POSTROUTING 0 -s ${P2PIPPREFIXSTART}.0/24 ! -d ${P2PIPPREFIXSTART}.0/24 -j MASQUERADE
		firewall-cmd -q --permanent --direct --remove-rule ipv4 nat POSTROUTING 0 -s ${P2PIPPREFIXSTART}.0/24 ! -d ${P2PIPPREFIXSTART}.0/24 -j MASQUERADE
...
tls-crypt tc.key
topology subnet
route $CLIENTPREFIXSTART.0.0 255.255.0.0
client-config-dir /etc/openvpn/ccd
server ${P2PIPPREFIXSTART}.0 255.255.255.0" > "$OVPN_CONF"
	# IPv6
	if [[ -z "$ip6" ]]; then
